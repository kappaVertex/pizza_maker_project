...
from maestro_pizza_maker.pizza import Pizza, weights_vector
...
def maximize_taste_penalty_price(
    ...
    model.objective = maximize(
        xsum(x[i] * (weights_vector[ingredients[i].value.type.value] * ingredients[i].value.fat.mean()) for i in range(len(ingredients))) - \
        lambda_param * xsum(x[i] * ingredients[i].value.price for i in range(len(ingredients)))
    )
